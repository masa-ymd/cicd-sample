...
CLOUDFRONT_DISTRIBUTION_ID: [開発環境のCloudFrontディストリビューションID] （Terraformデプロイ後に設定）
```
...
CLOUDFRONT_DISTRIBUTION_ID: [本番環境のCloudFrontディストリビューションID] （Terraformデプロイ後に設定）
```
...
4. CloudFrontディストリビューションIDを取得して`CLOUDFRONT_DISTRIBUTION_ID`シークレットに設定:
   ```bash
   ...
   # 出力から該当する環境のCloudFrontディストリビューションIDを特定し、GitHubシークレットに設定
   ```
